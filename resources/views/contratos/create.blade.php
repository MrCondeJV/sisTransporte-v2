...
@section('title', 'Nuevo Contrato de Transporte')
...
<div class="w-full max-w-4xl mx-auto space-y-6">
    <div class="flex items-center space-x-2 text-xs font-semibold text-slate-500 mb-1">
        ...
        <span>/</span>
        <span class="text-slate-800">Nuevo Contrato</span>
    </div>

    <div class="bg-white rounded-3xl border border-slate-200 shadow-sm p-6 sm:p-8">
        <div class="border-b border-slate-100 pb-5 mb-6">
            <h1 class="text-xl sm:text-2xl font-black text-slate-900 tracking-tight">Suscripción de Contrato de Transporte</h1>
            <p class="text-xs sm:text-sm text-slate-500 mt-1">Ingresa los datos jurídicos y comerciales del contrato para amparar los extractos FUEC.</p>
        </div>
        ...
        <form action="{{ route('contratos.store') }}" method="POST" class="space-y-6">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
                ...
                <div>
                    ...
                    <input type="text" name="contract_number" value="{{ old('contract_number') }}" required placeholder="Ej: CT-2026-0042"
                ...
                <div>
                    ...
                    <select name="contract_type" required class="block w-full px-4 py-2.5 border border-slate-300 rounded-xl text-sm focus:ring-2 focus:ring-blue-500 focus:border-blue-500">
                        <option value="Empresarial" {{ old('contract_type', 'Empresarial') === 'Empresarial' ? 'selected' : '' }}>Empresarial (Transporte de Empleados)</option>
                        <option value="Escolar" {{ old('contract_type') === 'Escolar' ? 'selected' : '' }}>Escolar (Estudiantes e Instituciones)</option>
                        <option value="Turismo" {{ old('contract_type') === 'Turismo' ? 'selected' : '' }}>Turismo y Recreación</option>
                        <option value="Salud" {{ old('contract_type') === 'Salud' ? 'selected' : '' }}>Usuarios del Servicio de Salud</option>
                        <option value="Grupo Especifico" {{ old('contract_type') === 'Grupo Especifico' ? 'selected' : '' }}>Grupo Específico de Usuarios / Ocasional</option>
                    </select>
                </div>
                ...
                <div class="sm:col-span-2">
                    ...
                    <textarea name="contract_object" rows="3" required placeholder="Descripción de las rutas, traslados o actividades objeto de la prestación..."
                        class="block w-full px-4 py-2.5 border border-slate-300 rounded-xl text-sm focus:ring-2 focus:ring-blue-500 focus:border-blue-500">{{ old('contract_object') }}</textarea>
                </div>
                ...
                <div>
                    ...
                    <input type="number" step="0.01" name="value" value="{{ old('value', 0) }}" required
                        class="block w-full px-4 py-2.5 border border-slate-300 rounded-xl text-sm font-mono focus:ring-2 focus:ring-blue-500 focus:border-blue-500">
                </div>
                ...
                <div>
                    ...
                    <select name="status" required class="block w-full px-4 py-2.5 border border-slate-300 rounded-xl text-sm focus:ring-2 focus:ring-blue-500 focus:border-blue-500">
                        <option value="Vigente" {{ old('status', 'Vigente') === 'Vigente' ? 'selected' : '' }}>Vigente</option>
