...
@section('title', 'Órdenes Completadas Sin Pagar (Cartera)')
...
<div class="w-full max-w-[1680px] mx-auto space-y-6">
    ...
    <div class="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
        <div>
            <div class="flex items-center space-x-2 text-xs font-semibold text-slate-500 mb-1">
                ...
                <span>/</span>
                <span class="text-slate-800">Cartera Pendiente</span>
            </div>
            <div class="flex items-center gap-3">
                <h1 class="text-2xl sm:text-3xl font-extrabold text-slate-900 tracking-tight flex items-center">
                    ...
                    Órdenes Completadas Sin Pagar (Cartera)
                </h1>
                <span class="px-3 py-1 rounded-full text-xs font-bold border bg-rose-50 text-rose-700 border-rose-200">
                    {{ $totalOrdenes }} servicios por cobrar
                </span>
            </div>
            <p class="text-sm text-slate-500 mt-1">Control de cuentas por cobrar y seguimiento de cartera a clientes con servicios ejecutados.</p>
        </div>
        ...
        <div class="flex items-center gap-3">
            ...
            <a href="{{ request()->fullUrlWithQuery(['export' => 'csv']) }}"
                class="inline-flex items-center justify-center px-5 py-2.5 rounded-xl text-sm font-bold text-white bg-rose-600 hover:bg-rose-700 shadow-sm shadow-rose-600/20 transition">
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
        <div class="bg-white rounded-2xl p-5 border border-slate-200 shadow-2xs">
            <span class="text-xs font-bold text-slate-500 uppercase tracking-wider">Saldo Total Pendiente por Cobrar</span>
            <div class="text-2xl sm:text-3xl font-black text-rose-600 mt-2">${{ number_format($totalPendiente, 2) }}</div>
            <span class="text-xs text-slate-400 mt-1 block">Monto en mora o en plazo de crédito</span>
        </div>

        <div class="bg-white rounded-2xl p-5 border border-slate-200 shadow-2xs">
            <span class="text-xs font-bold text-slate-500 uppercase tracking-wider">Cantidad de Servicios Pendientes</span>
            <div class="text-2xl sm:text-3xl font-black text-slate-900 mt-2">{{ number_format($totalOrdenes) }}</div>
            <span class="text-xs text-slate-400 mt-1 block">Servicios finalizados sin liquidación de pago</span>
        </div>

        <div class="bg-white rounded-2xl p-5 border border-slate-200 shadow-2xs">
            <span class="text-xs font-bold text-slate-500 uppercase tracking-wider">Valor Promedio por Cobrar</span>
            <div class="text-2xl sm:text-3xl font-black text-amber-600 mt-2">
                ${{ $totalOrdenes > 0 ? number_format($totalPendiente / $totalOrdenes, 2) : '0.00' }}
            </div>
            <span class="text-xs text-slate-400 mt-1 block">Promedio adeudado por orden</span>
        </div>
    </div>
    ...
    <div class="bg-white rounded-2xl border border-slate-200 p-5 shadow-2xs">
        <form method="GET" action="" class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 items-end">
            ...
            <div class="flex items-center gap-2">
                <button type="submit" class="w-full py-2.5 px-4 bg-rose-600 hover:bg-rose-700 text-white text-xs font-bold rounded-xl transition shadow-xs">
                    Filtrar Cartera
                </button>
                <a href="{{ route('gerencial.reportes.ordenes-sin-pagar') }}" class="py-2.5 px-3 bg-slate-100 hover:bg-slate-200 text-slate-600 text-xs font-bold rounded-xl transition">
                    Limpiar
                </a>
            </div>
        </form>
            ...
            <table class="w-full text-left text-sm">
                <thead class="bg-slate-50 border-b border-slate-200 text-xs font-bold text-slate-500 uppercase tracking-wider">
                    <tr>
                        ...
                        <th class="px-5 py-4">Ruta Realizada</th>
                        <th class="px-5 py-4">Fecha Servicio</th>
                        <th class="px-5 py-4">Vehículo & Conductor</th>
                        <th class="px-5 py-4">Factura Asociada</th>
                        <th class="px-5 py-4 text-right">Saldo Pendiente</th>
                ...
                <tbody class="divide-y divide-slate-100">
                    @forelse($ordenes as $o)
                        <tr class="hover:bg-slate-50/80 transition">
                            ...
                            <td class="px-5 py-4 text-xs">
                                @if($o->invoice_number)
                                    <span class="px-2 py-1 bg-amber-50 text-amber-700 font-mono font-bold rounded-lg border border-amber-200">
                                        {{ $o->invoice_number }}
                                    </span>
                                @else
                            ...
                            <td class="px-5 py-4 text-right font-extrabold text-rose-600 text-sm">
                    ...
                    @empty
                        <tr>
                            <td colspan="8" class="px-5 py-12 text-center text-slate-400 text-sm">
                                ¡Excelente! No hay órdenes con saldo pendiente registradas para este periodo.
                            </td>
                        </tr>
                    @endforelse
                </tbody>
