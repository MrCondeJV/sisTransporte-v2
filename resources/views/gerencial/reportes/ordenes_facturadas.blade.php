...
@section('title', 'Órdenes Facturadas')
...
<div class="w-full max-w-[1680px] mx-auto space-y-6">
    ...
    <div class="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
        <div>
            <div class="flex items-center space-x-2 text-xs font-semibold text-slate-500 mb-1">
                ...
                <span>/</span>
                <span class="text-slate-800">Órdenes Facturadas</span>
            </div>
            <div class="flex items-center gap-3">
                ...
                <span class="px-3 py-1 rounded-full text-xs font-bold border bg-blue-50 text-blue-700 border-blue-200">
                    {{ $totalOrdenes }} servicios con factura
                </span>
            </div>
            <p class="text-sm text-slate-500 mt-1">Control fiscal y auditoría de servicios con soporte de factura electrónica emitido.</p>
        </div>
        ...
        <div class="flex items-center gap-3">
            ...
            <a href="{{ request()->fullUrlWithQuery(['export' => 'csv']) }}"
                class="inline-flex items-center justify-center px-5 py-2.5 rounded-xl text-sm font-bold text-white bg-blue-600 hover:bg-blue-700 shadow-sm shadow-blue-600/20 transition">
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <div class="bg-white rounded-2xl p-5 border border-slate-200 shadow-2xs">
            <span class="text-xs font-bold text-slate-500 uppercase tracking-wider">Monto Total Facturado</span>
            <div class="text-2xl sm:text-3xl font-black text-blue-600 mt-2">${{ number_format($totalFacturado, 2) }}</div>
            <span class="text-xs text-slate-400 mt-1 block">Tarifas amparadas por facturación oficial</span>
        </div>

        <div class="bg-white rounded-2xl p-5 border border-slate-200 shadow-2xs">
            <span class="text-xs font-bold text-slate-500 uppercase tracking-wider">Cantidad de Órdenes Facturadas</span>
            <div class="text-2xl sm:text-3xl font-black text-slate-900 mt-2">{{ number_format($totalOrdenes) }}</div>
            <span class="text-xs text-slate-400 mt-1 block">Total de órdenes con factura registrada</span>
        </div>
    </div>
    ...
    <div class="bg-white rounded-2xl border border-slate-200 p-5 shadow-2xs">
        <form method="GET" action="" class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 items-end">
            ...
            <div class="flex items-center gap-2">
                ...
                <a href="{{ route('gerencial.reportes.ordenes-facturadas') }}" class="py-2.5 px-3 bg-slate-100 hover:bg-slate-200 text-slate-600 text-xs font-bold rounded-xl transition">
                    Limpiar
                </a>
            </div>
        </form>
            ...
            <table class="w-full text-left text-sm">
                <thead class="bg-slate-50 border-b border-slate-200 text-xs font-bold text-slate-500 uppercase tracking-wider">
                    <tr>
                        ...
                        <th class="px-5 py-4">No. Factura</th>
                        <th class="px-5 py-4">Cliente</th>
                        <th class="px-5 py-4">Ruta</th>
                        <th class="px-5 py-4">Fecha</th>
                        <th class="px-5 py-4">Vehículo</th>
                        <th class="px-5 py-4 text-right">Tarifa</th>
                ...
                <tbody class="divide-y divide-slate-100">
                    @forelse($ordenes as $o)
                        <tr class="hover:bg-slate-50/80 transition">
                            ...
                            <td class="px-5 py-4">
                                <span class="px-2.5 py-1 bg-blue-50 text-blue-700 font-mono font-bold rounded-lg border border-blue-200">
                                    {{ $o->invoice_number }}
                                </span>
                            </td>
                            <td class="px-5 py-4">
                            ...
                            <td class="px-5 py-4 text-right font-extrabold text-blue-600">
                    ...
                    @empty
                        <tr>
                            <td colspan="8" class="px-5 py-12 text-center text-slate-400 text-sm">
                                No se encontraron órdenes facturadas con los filtros seleccionados.
                            </td>
                        </tr>
                    @endforelse
                </tbody>
