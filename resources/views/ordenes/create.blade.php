...
<div class="w-full max-w-4xl mx-auto space-y-6">
    ...
    <div class="flex items-center justify-between">
        <div>
            <div class="flex items-center space-x-2 text-xs font-semibold text-slate-500 mb-1">
                ...
                <span>/</span>
                <span class="text-slate-800">Programar Despacho</span>
            </div>
            <h1 class="text-2xl sm:text-3xl font-extrabold text-slate-900 tracking-tight">Nueva Orden de Servicio</h1>
            <p class="text-sm text-slate-500 mt-1">Programa una ruta, asigna vehículo y conductor, y habilita la emisión del FUEC.</p>
        </div>
        <a href="{{ route('ordenes.index') }}" class="px-4 py-2 rounded-xl border border-slate-300 text-sm font-semibold text-slate-700 hover:bg-slate-100 transition">
            Volver
        </a>
    </div>
    ...
    <form action="{{ route('ordenes.store') }}" method="POST" class="space-y-6">
        @csrf
        ...
        <div class="bg-white rounded-3xl border border-slate-200 p-6 sm:p-8 shadow-sm space-y-5">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                    ...
                    <select name="client_id" required class="w-full py-2.5 px-3.5 border border-slate-300 rounded-xl text-sm">
                ...
                <div>
                    ...
                    <select name="contract_id" class="w-full py-2.5 px-3.5 border border-slate-300 rounded-xl text-sm">
        ...
        <div class="bg-white rounded-3xl border border-slate-200 p-6 sm:p-8 shadow-sm space-y-5"
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="text" name="origin" x-model="origin" required placeholder="Ej: Bogotá D.C. / Calle 100 # 15-20"
                ...
                <div>
                    ...
                    <input type="text" name="destination" x-model="destination" required placeholder="Ej: Girardot, Cundinamarca"
                        class="w-full px-3.5 py-2.5 border border-slate-300 rounded-xl text-sm">
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
                <div>
                    ...
                    <input type="datetime-local" name="scheduled_start_time" value="{{ old('scheduled_start_time', now()->format('Y-m-d\TH:i')) }}" required
                        class="w-full px-3.5 py-2.5 border border-slate-300 rounded-xl text-sm">
                </div>
                <div>
                    <label class="block text-xs font-bold uppercase tracking-wider text-slate-600 mb-1">Fecha y Hora Estimada de Llegada</label>
                    <input type="datetime-local" name="scheduled_end_time" value="{{ old('scheduled_end_time') }}"
                        class="w-full px-3.5 py-2.5 border border-slate-300 rounded-xl text-sm">
                </div>
                <div>
                    <label class="block text-xs font-bold uppercase tracking-wider text-slate-600 mb-1">Cantidad de Pasajeros *</label>
                    <input type="number" name="passengers_count" value="{{ old('passengers_count', 1) }}" required min="1"
                        class="w-full px-3.5 py-2.5 border border-slate-300 rounded-xl text-sm">
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="text" name="passenger_contact_name" value="{{ old('passenger_contact_name') }}" placeholder="Nombre del responsable"
                        class="w-full px-3.5 py-2.5 border border-slate-300 rounded-xl text-sm">
                </div>
                <div>
                    <label class="block text-xs font-bold uppercase tracking-wider text-slate-600 mb-1">Teléfono del Contacto</label>
                    <input type="text" name="passenger_contact_phone" value="{{ old('passenger_contact_phone') }}" placeholder="3001234567"
                        class="w-full px-3.5 py-2.5 border border-slate-300 rounded-xl text-sm">
                </div>
            </div>
        </div>
        ...
        <div class="bg-white rounded-3xl border border-slate-200 p-6 sm:p-8 shadow-sm space-y-5">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
                <div>
                    ...
                    <select name="vehicle_id" class="w-full py-2.5 px-3.5 border border-slate-300 rounded-xl text-sm font-mono">
                ...
                <div>
                    ...
                    <select name="status" required class="w-full py-2.5 px-3.5 border border-slate-300 rounded-xl text-sm">
                        ...
                        <option value="Asignada" {{ old('status') == 'Asignada' ? 'selected' : '' }}>Asignada</option>
            ...
            <div>
                ...
                <textarea name="service_notes" rows="2" placeholder="Indicaciones para el conductor o puntos de parada..."
                    class="w-full px-3.5 py-2.5 border border-slate-300 rounded-xl text-sm">{{ old('service_notes') }}</textarea>
            </div>
        </div>

        <div class="flex items-center justify-end space-x-3 pt-2">
            <a href="{{ route('ordenes.index') }}" class="px-5 py-2.5 rounded-xl border border-slate-300 text-sm font-semibold text-slate-700 hover:bg-slate-50 transition">
                Cancelar
            </a>
